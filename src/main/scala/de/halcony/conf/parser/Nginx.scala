...
object Nginx extends LogSupport {
  ...
  private[parser] def parseNonBreakCharacter[$ : P] : P[String] = P(CharsWhile(!Set('\n',' ','\t',';').contains(_)).!)

  private[parser] def parseFile[$ : P] : P[ConfigFile] = P(Start ~ parseExpr.rep ~ End).map(seq => ConfigFile(seq.toList,0,-1))

  private[parser] def parseExpr[$ : P] : P[Expression] = P(
    anyWhitespace.rep ~ (parseComment | parseAssignment | parseBlock)
  )

  private[parser] def parseComment[$: P]: P[CommentExpr] = P(Index ~ "#" ~/ CharsWhile(_ != '\n').! ~ "\n".?)
  ...
  private[parser] def parseAssignment[$ : P] : P[AssignmentExpr] = P(Index ~ parseName ~ !"{" ~ parseScalarList ~ ";")
    .map((startIndex,name,value) => AssignmentExpr(name,if value.values.length == 1 then value.values.head else value,startIndex,-1))//.log

  private[parser] def parseBlock[$ : P] : P[BlockExpr] = P(Index ~ parseName ~ "{" ~ parseExpr.rep ~ "}")
    .map((indexStart,nameExpr,exprs) => BlockExpr(Some(nameExpr),exprs.toList,indexStart,-1))//.log
